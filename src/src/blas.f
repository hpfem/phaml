c The BLAS source was obtained first from ARPACK to make sure we have the
c version it requires, and then missing routines obtained from Netlib.

c The following BLAS source files were obtained from Netlib via GAMS
...
c The following BLAS source files were obtained from the ARPACK distribution
...
c
c     WFM modified 4/5/07, array(1) declarations changed to array(*)
...
c end of BLAS routines taken from ARPACK distribution
