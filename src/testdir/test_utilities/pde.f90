...
cyy(1,1) = 1.0_my_real
c(1,1) = 0.0_my_real
rs(1) = -power*(power-1)*(x**(power-2)+y**(power-2))
...
if (kernel == 2) then
   phaml_integral_kernel = x
else
   phaml_integral_kernel = 1.0_my_real
endif
