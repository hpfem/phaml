...
!
! This version illustrates periodic boundary conditions.
!----------------------------------------------------
...
cyy(1,1) = 1.0_my_real
c(1,1) = 0.0_my_real
rs(1) = (4*pi**2*x**2 - 2)*sin(2*pi*y + a)
...
!
!    u = rs(x,y) or  u  + c(x,y)*u = rs(x,y)  or  periodic
!                     n
!
...
! the second condition with c/=0, and PERIODIC for periodic.
!
! Periodic conditions are illustrated in this file for a rectangular domain.
! Periodic conditions may apply to the top and bottom sides, left and right
! sides, or both.  If it is both then a Dirichlet condition must apply to the
! corners (which necessarily take the same value).  When itype is PERIODIC,
! the values returned in c and g are irrelevant.
!----------------------------------------------------
