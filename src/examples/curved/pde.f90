...
! bmark==1, i.e. piece 1, is the semicircle.  Impose Dirichlet conditions.
! bmark==2, i.e. piece 2, is the point (-1,0).  The point is included as a
!           separate piece to impose Dirichlet conditions at that point
!           (otherwise it would be part of the next piece).
! bmark==3, i.e. piece 3, is the segment of the x axis.  Impose Natural b.c.
...
! Define a semi-circle domain.  Piece 1 is a semi-circle in the upper
! half plane from (1,0) to (-1,0).  Piece 2 is the point (-1,0).
! Piece 3 is a line from (-1,0) to (1,0).
! The parameter ranges, given in subroutine boundary_param, are
! (0,pi), (0,0) and (-1,1).
...
start(1) = 0.0_my_real
finish(1) = 4*atan(1.0_my_real) ! pi
start(2) =  0.0_my_real
finish(2) = 0.0_my_real
start(3) = -1.0_my_real
finish(3) = 1.0_my_real
