...
!
! This version illustrates a parabolic equation.
!----------------------------------------------------
...
select case (bmark)
   ...
   case (1,2,3)
      itype = DIRICHLET
      rs = exp(-100*(y-(2+sin(10*t+.5_my_real))/4)**2)
...
iconds = exp(-100*(y-(2+sin(.5_my_real))/4)**2)*exp(-100*x**2)
